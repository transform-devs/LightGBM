...
class Parser {
 public:
  ...
  /*!
  * \brief constructor for customized parser.
  * \param Parser config content, not path because need to save/load the config along with model string.
  */
...
class ParserFactory {
 private:
  ParserFactory() {}
  std::map<std::string, std::function<Parser*(std::string)>> _object_map;
